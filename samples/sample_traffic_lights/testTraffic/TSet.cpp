...
// конструктор преобразовани€ типа
...
int TSet::IsMember(const int Elem) const // элемент множества?
{
    if (Elem < 0 || Elem >= MaxPower) {
        throw out_of_range("Ёлемент вне юниверса множества!");
    }
...
{
    if (Elem < 0 || Elem >= MaxPower) {
        throw out_of_range("Ёлемент ставитс€ за пределами множества");
    }
...
{
    if (Elem < 0 || Elem >= MaxPower) {
        throw out_of_range("Ёлемент за пределами множества удал€етс€");
    }
...
{
    //if (MaxPower != s.MaxPower) { throw std::runtime_error("Ќельз€ сравнить множества разных размеров!"); }
...
{
    if (Elem < 0) {
        throw out_of_range("Ёлемент не может быть отрицательным");
    }
    TSet res = *this;
    if (Elem >= MaxPower) {
        TSet newSet(Elem + 1);
